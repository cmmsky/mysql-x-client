...
/**
 * @Author: cmmsky
 * @Date: Created in 15:02 2021/4/9
 * @Description:
 * @Modified by:
 */
public class AbstractRowsEventData {
    ...
    protected Object [] decodeRows(TableMapEventData tableMapEventData, ColumnSet columnSet, MySQLMessage mm) {
        ...
        for (int i = 0, numberOfSkippedColumns = 0; i < columnTypes.length; i++) {
            ...
            if (!nullColumns.get(index)) {
                int typeCode = columnTypes[i] & 0xFF, meta = metadata[i], length = 0;
    ...
    protected Long decodeDatetime(MySQLMessage mm) {
        int[] split = split(mm.readLong(), 100, 6);
